  <!-- Sidebar -->
  <div class="bg-light border-right" id="sidebar-wrapper">
    <div class="sidebar-heading">
      WISDOM PARK <br>
      <span style="color:gray;font-size:12px">Admin Panel</span>
    </div>
    <div class="list-group list-group-flush">
      <a href="#" class="list-group-item list-group-item-action bg-light"><i class="fa fa-tachometer-alt"></i> &nbsp;Dashboard</a>
      <a href="admin-details" class="list-group-item list-group-item-action bg-light"><i class="fa fa-info"></i> &nbsp;Website Details</a>
      <a href="admin-gallery" class="list-group-item list-group-item-action bg-light"><i class="fa fa-photo-video"></i> &nbsp;Gallery</a>
      <a href="#" class="list-group-item list-group-item-action bg-light"><i class="fa fa-calendar-check"></i> &nbsp;Events</a>
      <a href="admin-newsletter" class="list-group-item list-group-item-action bg-light"><i class="fa fa-envelope"></i> &nbsp;Newsletter Subsriber</a>
      <a href="#" class="list-group-item list-group-item-action bg-light"><i class="fa fa-user"></i> &nbsp;User Management</a>

    </div>
  </div>
  <!-- /#sidebar-wrapper -->